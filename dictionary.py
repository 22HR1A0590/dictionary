...
'''2)The program takes two dictionaries and concatenates them into one dictionary. 
Problem Solution:
...
'''3) The program takes a number from the user and generates a dictionary that contains numbers (between 1 and n) in the form (x,x*x). 
Problem Solution:
...
my_dict = {}
for i in range(n):
    key = i + 1  # Using 1-based index as keys
